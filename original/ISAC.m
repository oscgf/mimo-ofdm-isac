...
waveLength = freq2wavelen(carrierFrequency);        % Wavelength (m)
bandwidth = 100e6;                                  % Bandwidth (Hz)
sampleRate = bandwidth;                             % Sample rate (Hz)

% Transmitter Parameters
peakPower = 1.0;                                    % Peak power (W)
transmitter = phased.Transmitter('PeakPower', peakPower, 'Gain', 0);  % MIMO transmitter
...
rxArray = phased.ULA(Nrx, waveLength/2, 'Element', element);    % Receive uniform linear array

%% ISAC Scenario

% Transmitter location and orientation
txPosition = [0; 0; 0];              % Tx location
txOrientationAxis = eye(3);          % Tx orientation axis

% Receiver location and orientation
rxPosition = [80; 60; 0];            % Rx location
rxOrientationAxis = rotz(-90);       % Rx orientation axis (rotated -90 degrees)

% Maximum path lengths and target velocities
maxPathLength = 300;                 % Maximum path lengths between Tx and Rx (m)
maxVelocity = 50;                    % Maximum target velocity (m/s)     
...
Mt = floor(1/(2*maxDopplerShift*Tofdm));            % Doppler sampling parameter

maxDelay = range2time(maxPathLength)/2;              % Maximum delay
Mf = floor(1/(2*subcarrierSpacing*maxDelay));        % Range sampling parameter

frameLength = Mt;                                    % Frame length

subframeALength = frameLength - Ntx;                 % Subframe A length
subframeBLength = Ntx;                               % Subframe B length
...
numPreambleSubcarriers = numel(idxs);               % Number of preamble subcarriers
...
% Create OFDM demodulator for subframe A
...
% Create OFDM modulator for subframe B
...
% Create OFDM demodulator for subframe B
...
Nframe = 6;                                                            % Total number of transmitted OFDM frames
